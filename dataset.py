...
class CoCoDataset(data.Dataset):
    def __init__(self, vocab_file, vocab_thres, start_word,
        ...
        if self.mode == 'train' or self.mode == 'val':
            ...
            vocab_tokens = [nltk.tokenize.word_tokenize(str(self.coco.anns[self.idxs[id]]['caption']).lower()) for id in tqdm(np.arange(len(self.idxs)))]
    ...
    def __getitem__(self, idx):
        if self.mode == 'train' or self.mode == 'val':
            ...
            path = self.coco.loadImgs(img_id)[0]['file_name'] # 'file_name': '000000380706.jpg'
            ...
            tokens = nltk.tokenize.word_tokenize(str(caption).lower()) # list of 1 caption-token or more
            ...
            path = self.paths[idx]
            PIL_img = Image.open(os.path.join(self.img_folder, path)).convert('RGB')
    ...
    def get_train_indices(self):
        ...
        all_indices = np.where([self.cap_len[i] == sel_length for i in np.arange(len(self.cap_len))])[0]
